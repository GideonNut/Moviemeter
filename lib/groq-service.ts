...
// Initialize the Groq client with API key from environment variables - with fallback
const getGroqClient = () => {
...
export async function getPersonalizedRecommendations(preferences: string, count = 3): Promise<MovieRecommendation[]> {
  try {
    ...
    // If no Groq client, return mock data
    if (!groq) {
...
`
    ...
    return completion.choices[0]?.message?.content || getMockMovieAnalysis(movieTitle)
  } catch (error) {
...
`
    ...
    return parsedResponse.questions || getMockDiscussionQuestions(movieTitle, count)
  } catch (error) {
...
// Mock data functions for when Groq API is not available
