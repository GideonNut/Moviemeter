...
function MovieCard({ id, title, description, address }: MovieCardProps) {
  ...
  const baseUrl =
    process.env.NEXT_PUBLIC_BASE_URL || "https://moviemeter12.vercel.app";
  ...
  return (
    <div className="border border-zinc-800 p-4 rounded-lg hover:bg-zinc-900 w-full text-center">
      <div className="aspect-[2/3] relative mb-4 rounded-md overflow-hidden bg-black">
        <Image
          src={`/placeholder-8k514.png?height=450&width=300&text=${encodeURIComponent(
            currentMovie.title
          )}`}
  ...
  const handleVote = async (voteType: boolean) => {
    ...
    try {
      ...
      const transaction = prepareContractCall({
        ...
        args: [BigInt(id), voteType], // Convert 'id' to bigint
      });
