...
export async function GET(req: NextRequest) {
  try {
    ...
    // Calculate earnings (mock calculation based on votes and streaks)
    const topEarners = Object.values(userStats)
      .map(user => ({
        ...user,
        earnings: Math.floor(user.totalVotes * 10 + user.streak * 5) // Simple earnings calculation
      }))
